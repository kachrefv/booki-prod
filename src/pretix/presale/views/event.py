...
class EventIndex(EventViewMixin, EventListMixin, CartMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if not self.request.event.has_subevents or self.subevent:
            # Fetch ALL items by setting require_seat to None
            ...
            # Provide the seating plan URL to the template if it exists
            seating_plan = self.subevent.seating_plan if self.subevent else self.request.event.seating_plan
            context['seating_plan_url'] = None
            if seating_plan:
                seating_plan_kwargs = {'cart_namespace': kwargs.get('cart_namespace') or ''}
                if self.subevent:
                    seating_plan_kwargs['subevent'] = self.subevent.pk
                context['seating_plan_url'] = eventreverse(self.request.event, 'presale:event.seatingplan', kwargs=seating_plan_kwargs)
            ...
            context['allfree'] = all(
                ...
                for item in items if not item.has_variations and not item.requires_seat
            ) and all(
                all(
            ...
            # Regroup all items by category
            context['items_by_category'] = item_group_by_category(items)

            # MODIFICATION START: Force the 'Add to cart' button to be visible if any products are shown
            has_regular_items_available = any(not item.requires_seat for item in items)
            has_seated_items = any(item.requires_seat for item in items)
            if has_regular_items_available or has_seated_items:
                context['display_add_to_cart'] = True
            else:
